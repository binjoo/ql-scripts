...
const $ = new Env('HiFiNi签到');
const COOKIE_NAME = 'NM_HIFINI_COOKIE';
...
async function getSignToken () {
  return await axios.get('https://www.hifini.com', config).then((response) => {
...
async function getSignDetail () {
  return await axios.get('https://www.hifini.com/sg_sign.htm', config).then(async (response) => {
    const root = cheerio.load(response.data);
    /**
     * 0 今日签到排名
     * 1 用户名
